...
// once from a CSV file, or clear out the current store type's
// existing catalog first -- e.g. replacing a demo/seed catalog with a
// real one. Moved here from the POS catalog toolbar -- it's an
...
export async function mountBulkImport(container) {
  ...
  container.appendChild(el('div', { class: 'settings-section' }, [
    ...
    el('p', { class: 'settings-hint' },
      'Permanently deletes every product belonging to your CURRENT store type only (other store types\u2019 catalogs are never touched). ' +
