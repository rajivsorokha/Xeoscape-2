...
export function openPaymentDialog({ cartManager, currentUserId, discount = 0, customerId = null, onComplete }) {
  ...
  const creditHint = el('div', { class: 'settings-hint', style: 'display:none;' },
    customerId
      ? 'Enter what\u2019s being collected now (0 is fine if paying in full later) \u2014 the remainder is tracked as Due on this customer\u2019s account.'
      : '\u26A0 Select a customer above first \u2014 the amount due needs an account to bill it to.'
  );
  ...
  function updateChangeBar() {
    ...
    if (paymentMethod === 'card') {
      ...
      // Credit is the payment method (goods now, pay later -- not a
      ...
      if (!customerId) {
        ...
        changeBar.textContent = `Due: ${formatMoney(Math.abs(change), symbol)} (added to customer\u2019s account)`;
        changeBar.classList.remove('insufficient');
      } else {
        changeBar.textContent = `Paid in full \u2014 nothing due. Change: ${formatMoney(change, symbol)}`;
        changeBar.classList.remove('insufficient');
      }
    } else if (change < 0) {
